...
def parse_args():
    ...
    parser.add_argument('--trainset_dir', type=str, default='/data/data-home/hekun/Steore_SR/Stereo_image/Flickr1024_patches')
    parser.add_argument('--model_name', type=str, default='model_hk_cosin')
    parser.add_argument('--load_pretrain', type=bool, default=False)
    parser.add_argument('--model_path', type=str, default='checkpoint_model_2_x4/iPASSR_4xSR_epoch10.pth.tar')
    parser.add_argument('--checkpoint', type=str, default='checkpoint_model1_x4_hk_cosin/')
...
def train(train_loader, cfg):
    ...
    scheduler = CosineAnnealingWithRestartsLR(optimizer, T_max=100, eta_min=1e-6)
    loss_epoch = []
    loss_list = []
    ...
    for idx_epoch in range(cfg.start_epoch, cfg.n_epochs):
        ...
        for idx_iter, (HR_left, HR_right, LR_left, LR_right) in enumerate(train_loader):
            ...
            scheduler.step()# After every batch finish, npdate learning rate
        #scheduler.step()# After every epoch finish, npdate learning rate
        ...
        if idx_epoch % 1 == 0:
            ...
            torch.save({'epoch': idx_epoch + 1, 'state_dict': net.state_dict()},
                       cfg.checkpoint + cfg.model_name + '_' + str(cfg.scale_factor) + 'xSR_epoch' + str(idx_epoch + 1) + '.pth.tar')
            loss_epoch = []
...
def main(cfg):
    ...
    print("scale_factor:x{},batch_size:{},lr:{},n_epochs:{}".format(cfg.scale_factor,cfg.batch_size,cfg.lr,cfg.n_epochs))
